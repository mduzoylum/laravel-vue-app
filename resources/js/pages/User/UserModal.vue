        ...
        <div class="modal-dialog" role="document">
            <div class="modal-content">
                <div class="modal-header">
                    <h5 class="modal-title" v-text="item.id > 0 ?'Kullanıcı Düzenle' : 'Yeni Kullanıcı'">
                ...
                <div class="modal-footer">
                    <button class="btn btn-primary" v-text="item.id > 0 ? 'Güncelle' : 'Kaydet'"
...
export default {
    ...
    methods: {
        saveItem() {
            if (this.item.id > 0) {
                axios.put('/users/'+this.item._id, this.item)
                    .then(response => {
                        if (response.data.success) {
                            this.$emit('onSaved', this.item);
                            $("#userModal").modal("hide");
                            toastr.success("Kayıt güncellendi","Kullancı");
                        }
                    })
                    .catch(error => {
